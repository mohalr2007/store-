...
export function CartDrawer({ isOpen, onClose }: CartDrawerProps) {
  ...
  return (
    <>
      ...
      <div className={cn(
        "fixed top-0 z-50 flex h-full w-full max-w-md flex-col bg-[var(--bg-card)] shadow-2xl transition-transform duration-300",
        isOpen ? "translate-x-0" : "translate-x-full"
      )}>
        {/* Header */}
        <div className="flex items-center justify-between border-b border-[var(--border)] px-5 py-4">
          <h2 className="text-base font-semibold">سلة المشتريات ({itemCount})</h2>
          <button onClick={onClose} className="rounded-lg p-1.5 text-[var(--muted-foreground)] hover:bg-[rgba(69,212,232,0.08)]">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <ShoppingBag className="mb-3 h-12 w-12 text-[var(--muted-foreground)]/20" />
              <p className="text-sm text-[var(--muted-foreground)]">سلة المشتريات فارغة</p>
              <Link href="/products" onClick={onClose} className="btn-primary mt-4 h-10 px-4 text-sm">
                تصفح المنتجات
              </Link>
            </div>
          ) : (
            <div className="space-y-4">
              {items.map((item) => {
                ...
                return (
                <div key={itemKey} className="flex gap-4 rounded-xl border border-[var(--border)] p-3">
                  <div className="h-20 w-20 shrink-0 overflow-hidden rounded-lg bg-[var(--bg-card-2)]">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          className="flex h-7 w-7 items-center justify-center rounded-lg border border-[var(--border)] hover:bg-[rgba(69,212,232,0.08)]"
                        >
        ...
        {items.length > 0 && (
          <div className="border-t border-[var(--border)] px-5 py-4">
            ...
            <Link
              href="/checkout"
              onClick={onClose}
              className="btn-primary h-11 w-full text-sm shadow-lg"
            >
              متابعة الطلب
            </Link>
